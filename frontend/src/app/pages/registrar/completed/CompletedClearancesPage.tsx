...
import { mockCompletedClearances } from "./data/mockCompletedData";
import { CompletedClearance } from "./data/types";
import { CompletedClearanceWorkspace } from "./components/workspace/CompletedClearanceWorkspace";

export function CompletedClearancesPage() {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedClearance, setSelectedClearance] = useState<CompletedClearance | null>(null);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-sm text-slate-500 mb-1">
              ...
              <span>/</span>
              <span className="text-slate-900 font-medium">Completed Clearances</span>
            </div>
          ...
          <div className="flex items-center gap-3">
            <div className="relative w-64 hidden sm:block">
              ...
              <input 
                ...
                className="w-full pl-9 pr-4 py-2 bg-slate-100 border-transparent rounded-lg text-sm focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all"
              />
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col">
          ...
          <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50/50 rounded-t-xl">
            <div className="relative w-full sm:w-80">
              ...
              <input 
                type="text"
                placeholder="Search by ID, Name, Cert #, Token..."
                className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all"
              />
            </div>
            <CompletedFilters />
          </div>
          ...
          {/* Archive Table */}
          <CompletedArchiveTable 
            data={mockCompletedClearances} 

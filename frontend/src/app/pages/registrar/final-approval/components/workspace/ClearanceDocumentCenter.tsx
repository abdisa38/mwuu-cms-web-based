          ...
          <div key={doc.id} className="border border-slate-200 rounded-xl overflow-hidden hover:border-blue-300 transition-colors group">
            ...
            <div className="p-4 bg-white">
              <div className="flex items-start justify-between gap-2">
                ...
                {doc.status === 'Verified' && <CheckCircle className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />}
              </div>
              <div className="mt-2 space-y-1">
                <p className="text-xs text-slate-500 flex justify-between"><span>Type:</span> <span className="font-medium text-slate-700">{doc.type}</span></p>
                <p className="text-xs text-slate-500 flex justify-between"><span>Uploaded:</span> <span className="font-medium text-slate-700">{new Date(doc.uploadDate).toLocaleDateString()}</span></p>
              </div>

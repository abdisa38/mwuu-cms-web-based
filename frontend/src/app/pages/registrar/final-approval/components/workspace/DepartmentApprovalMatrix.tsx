...
export function DepartmentApprovalMatrix({ clearance }: DepartmentApprovalMatrixProps) {
        ...
        <table className="w-full text-sm text-left">
          <thead className="text-xs text-slate-500 uppercase bg-white border-b border-slate-200">
            <tr>
              ...
              <th className="px-5 py-3 font-semibold">Responsible Officer</th>
              <th className="px-5 py-3 font-semibold">Decision Date</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {clearance.departments.map((dept) => (
              <tr key={dept.id} className="hover:bg-slate-50 transition-colors bg-white">
                ...
                <td className="px-5 py-4 text-right">
                  <Button variant="outline" size="sm" className="h-8 bg-white border-slate-200">Details</Button>
                </td>
              </tr>
            ))}
          </tbody>
